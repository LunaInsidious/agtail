...
意図的にオフライン(LiteLLM もバックエンドも呼びません)です。自分の bundle をインポートして閲覧・検索できますが、**コスト**はサンプルのモデル分のみ、**プラグイン帰属**はサンプルのプラグインのみに制限されます。完全な価格表と自分のプラグインを使うにはローカルで agtail を動かしてください。
:::
